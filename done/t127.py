...
def direction2(q,gimg):
	...
	intr=21.0
	the_range=[]
	for x in range(height-1,0,-21):
		the_range.append([x,0])
	for y in range(0,width,21):
	...
	for t_range in the_range:
		...
		tx=t_range[0]
		ty=t_range[1]
		q_tmp += "G0 X" + str(tx/intr) + " Y" + str(ty/intr) + "\n"
		print "X %d Y %d" %(tx,ty)
